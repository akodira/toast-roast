...
const COLUMN_MAP = {
  ...
  isfeatured: "IsFeatured", imageposition: "ImagePosition", note: "Note",
  imagelayout: "ImageLayout", imagefit: "ImageFit", imagefocusx: "ImageFocusX", imagefocusy: "ImageFocusY",
  menu_pdf: "menu_pdf",
  tableid: "TableId", occupiedby: "OccupiedBy", occupiedname: "OccupiedName", occupiedat: "OccupiedAt",
  pinhash: "PinHash", pinattempts: "PinAttempts", pinlockeduntil: "PinLockedUntil",
  invoiceid: "InvoiceId", ispaid: "IsPaid", paidat: "PaidAt", tablename: "TableName", ordercount: "OrderCount",
  ...
  customername: "CustomerName", catname: "CatName",
  receivedat: "ReceivedAt", preparingat: "PreparingAt", readyat: "ReadyAt", deliveredat: "DeliveredAt",
  callid: "CallId", phone: "Phone", resolvedat: "ResolvedAt",
  ratingid: "RatingId", score: "Score", comment: "Comment",
  sideoptions: "SideOptions", sides: "Sides", sidelimit: "SideLimit",
  section: "Section", allowed: "Allowed",
  groupinvoiceid: "GroupInvoiceId", groupispaid: "GroupIsPaid", groupoccupiedat: "GroupOccupiedAt",
  isreserved: "IsReserved", reservedphone: "ReservedPhone", reservedname: "ReservedName",
  branchid: "BranchId", slug: "Slug", address: "Address", ismain: "IsMain",
  openinghours: "OpeningHours", mapurl: "MapUrl", mapembed: "MapEmbed", menupdfurl: "MenuPdfUrl",
};
...
async function migrate(p) {
  const schema = `
    ...
    ALTER TABLE Categories ADD COLUMN IF NOT EXISTS ImageUrl VARCHAR(400) NULL;
    ALTER TABLE Categories ADD COLUMN IF NOT EXISTS ImagePosition VARCHAR(20) NOT NULL DEFAULT 'center';
    ALTER TABLE Categories ADD COLUMN IF NOT EXISTS Note VARCHAR(400) NULL;
    ALTER TABLE Categories ADD COLUMN IF NOT EXISTS ImageLayout VARCHAR(10) NOT NULL DEFAULT 'overlay';
    ALTER TABLE Categories ADD COLUMN IF NOT EXISTS ImageFit VARCHAR(10) NOT NULL DEFAULT 'cover';
    ALTER TABLE Categories ADD COLUMN IF NOT EXISTS ImageFocusX INT NOT NULL DEFAULT 50;
    ALTER TABLE Categories ADD COLUMN IF NOT EXISTS ImageFocusY INT NOT NULL DEFAULT 50;
    ...
    ALTER TABLE MenuItems ADD COLUMN IF NOT EXISTS IsFeatured BOOLEAN NOT NULL DEFAULT FALSE;
    ...
    ALTER TABLE Tables ADD COLUMN IF NOT EXISTS OccupiedBy VARCHAR(50) NULL;
    ALTER TABLE Tables ADD COLUMN IF NOT EXISTS OccupiedName VARCHAR(200) NULL;
    ALTER TABLE Tables ADD COLUMN IF NOT EXISTS OccupiedAt TIMESTAMPTZ NULL;
    ALTER TABLE Tables ADD COLUMN IF NOT EXISTS PinHash VARCHAR(120) NULL;
    ALTER TABLE Tables ADD COLUMN IF NOT EXISTS PinAttempts INT NOT NULL DEFAULT 0;
    ALTER TABLE Tables ADD COLUMN IF NOT EXISTS PinLockedUntil TIMESTAMPTZ NULL;
    ALTER TABLE Tables ADD COLUMN IF NOT EXISTS IsReserved BOOLEAN NOT NULL DEFAULT FALSE;
    ALTER TABLE Tables ADD COLUMN IF NOT EXISTS ReservedPhone VARCHAR(50) NULL;
    ALTER TABLE Tables ADD COLUMN IF NOT EXISTS ReservedName VARCHAR(200) NULL;
    CREATE TABLE IF NOT EXISTS Invoices (
      InvoiceId SERIAL PRIMARY KEY,
      TableId INT NOT NULL REFERENCES Tables(TableId),
      CustomerId INT NOT NULL REFERENCES Customers(CustomerId),
      OccupiedAt TIMESTAMPTZ NOT NULL,
      IsPaid BOOLEAN NOT NULL DEFAULT FALSE,
      PaidAt TIMESTAMPTZ NULL,
      CreatedAt TIMESTAMPTZ NOT NULL DEFAULT NOW(),
      UNIQUE(TableId, CustomerId, OccupiedAt)
    );
    -- (6) Free multi-select side items / add-ons (Jul 2026).
    -- MenuItems.SideOptions holds the option names, one per line. SideLimit is
    -- how many the customer may pick (0 = unlimited). OrderDetails.Sides holds
    -- the customer's chosen options for that line, comma-separated.
    ALTER TABLE MenuItems ADD COLUMN IF NOT EXISTS SideOptions TEXT NULL;
    ALTER TABLE MenuItems ADD COLUMN IF NOT EXISTS SideLimit INT NOT NULL DEFAULT 0;
    ALTER TABLE OrderDetails ADD COLUMN IF NOT EXISTS Sides VARCHAR(500) NULL;
    -- (7) Per-user section overrides (Jul 2026). Allowed=true grants a section
    -- the user's role wouldn't have; Allowed=false revokes one it would.
    ...
    -- Customer-experience batch (Jul 2026) --------------------------------
    -- (2) Per-status timestamps for the order timeline. NULL until reached.
    ALTER TABLE Orders ADD COLUMN IF NOT EXISTS ReceivedAt  TIMESTAMPTZ NULL;
    ALTER TABLE Orders ADD COLUMN IF NOT EXISTS PreparingAt TIMESTAMPTZ NULL;
    ALTER TABLE Orders ADD COLUMN IF NOT EXISTS ReadyAt     TIMESTAMPTZ NULL;
    ALTER TABLE Orders ADD COLUMN IF NOT EXISTS DeliveredAt TIMESTAMPTZ NULL;
    -- (4) Per-line special instructions.
    ALTER TABLE OrderDetails ADD COLUMN IF NOT EXISTS Note VARCHAR(300) NULL;
    -- (3) Call-waiter requests.
    ...
    CREATE INDEX IF NOT EXISTS idx_waitercalls_status ON WaiterCalls(Status);
    -- (9) Service ratings.
    ...
    ALTER TABLE Categories ADD COLUMN IF NOT EXISTS BranchId INT NULL REFERENCES Branches(BranchId);
    ALTER TABLE MenuItems ADD COLUMN IF NOT EXISTS BranchId INT NULL REFERENCES Branches(BranchId);
    ALTER TABLE Tables ADD COLUMN IF NOT EXISTS BranchId INT NULL REFERENCES Branches(BranchId);
    ALTER TABLE Orders ADD COLUMN IF NOT EXISTS BranchId INT NULL REFERENCES Branches(BranchId);
    ALTER TABLE Invoices ADD COLUMN IF NOT EXISTS BranchId INT NULL REFERENCES Branches(BranchId);
    ALTER TABLE Branches ADD COLUMN IF NOT EXISTS IsMain BOOLEAN NOT NULL DEFAULT FALSE;
    ALTER TABLE Branches ADD COLUMN IF NOT EXISTS Email VARCHAR(200) NULL;
    ALTER TABLE Branches ADD COLUMN IF NOT EXISTS OpeningHours VARCHAR(300) NULL;
    ALTER TABLE Branches ADD COLUMN IF NOT EXISTS MapUrl VARCHAR(600) NULL;
    ALTER TABLE Branches ADD COLUMN IF NOT EXISTS MapEmbed TEXT NULL;
    ALTER TABLE Branches ADD COLUMN IF NOT EXISTS MenuPdfUrl VARCHAR(600) NULL;
    ...
    ALTER TABLE Tables DROP CONSTRAINT IF EXISTS tables_name_unique;
    DROP INDEX IF EXISTS tables_name_key;
    CREATE UNIQUE INDEX IF NOT EXISTS uq_tables_name_branch ON Tables(Name, BranchId);
  `;
  // Run each statement on its own. A single multi-statement query runs in one
  // implicit transaction, so ONE incompatible statement rolls back and throws
  // the whole migration — which 500s every page. Splitting means each
  // idempotent CREATE/ALTER stands alone: a statement that's already applied
  // or harmlessly fails (e.g. "already exists") is logged and skipped instead
  // of taking the site down. Comments and blank fragments are ignored.
  const statements = schema
    .split(/;\s*(?:\r?\n|$)/)                     // split on ; at line ends
  ...
  for (const stmt of statements) {
    try {
      ...
      // Idempotent DDL that legitimately no-ops (duplicate object, etc.) is
      // fine to skip; anything else we log with the offending statement so the
      // cause is visible in the server logs instead of a blank 500.
...
async function seed(p) {
  ...
  // Backfill: give every existing user their current single RoleId as a row
  // in the new multi-role table, so upgrading to multi-role doesn't strip
  // anyone's access. Safe to run every deploy (ON CONFLICT DO NOTHING).
  await db.prepare(`INSERT INTO UserRoles (UserId, RoleId) SELECT UserId, RoleId FROM Users
    ON CONFLICT (UserId, RoleId) DO NOTHING`).run();

  // Multi-branch foundation (Stage 1). Create the first branch once and assign
  // ALL existing data to it, so a single-branch database becomes branch-aware
  // without losing anything. Idempotent: the branch is only created when none
  // exist, and the backfills only touch rows whose BranchId is still NULL, so
  // re-running on every deploy is a no-op after the first time.
  try {
    const anyBranch = await db.prepare("SELECT BranchId FROM Branches ORDER BY BranchId LIMIT 1").get();
    let firstBranchId = anyBranch ? anyBranch.BranchId : null;
    if (!firstBranchId) {
      // Seed the current restaurant as the first branch. Tax/service are left
      // NULL here so the app keeps using the existing global Settings values as
      // the default until you set per-branch rates in the admin.
      const created = await db.prepare(
        `INSERT INTO Branches (Name, Slug, DisplayOrder, IsActive) VALUES ($1,$2,0,true) RETURNING BranchId AS id`
      ).run("Toast & Roast — Cairo", "cairo");
      firstBranchId = created.lastInsertRowid;
    }
    if (firstBranchId) {
      // Assign every pre-existing row (created before branches existed) to the
      // first branch. Only NULLs are touched, so later branches are unaffected.
      for (const tbl of ["Categories", "MenuItems", "Tables", "Orders", "Invoices"]) {
        await db.prepare(`UPDATE ${tbl} SET BranchId=$1 WHERE BranchId IS NULL`).run(firstBranchId);
      }
      // Ensure exactly one Main branch for the public website. If none is Main
      // yet, make the first branch Main (Cairo, the original site).
      const anyMain = await db.prepare("SELECT BranchId FROM Branches WHERE IsMain=true LIMIT 1").get();
      if (!anyMain) await db.prepare("UPDATE Branches SET IsMain=true WHERE BranchId=$1").run(firstBranchId);
    }
  } catch (err) {
    console.error("[seed] branch foundation skipped:", err.message);
  }

  // Cleanup: earlier versions created a new Customer row on every order (no
  // dedup) and never removed one when its order was deleted — this sweeps up
  // leftovers with zero orders attached. No-op once they're gone.
  //
  // ORDER MATTERS: Invoices has a FK to Customers, so an invoice belonging to
  // an order-less customer must go FIRST, or the customer DELETE below raises
  // invoices_customerid_fkey and — because this runs during seeding — takes
  // the whole site down with a 500 on every page. An invoice whose customer
  // has no orders has nothing left to bill, so removing it is correct.
  await db.prepare("DELETE FROM Invoices WHERE CustomerId NOT IN (SELECT DISTINCT CustomerId FROM Orders)").run();
  await db.prepare("DELETE FROM Customers WHERE CustomerId NOT IN (SELECT DISTINCT CustomerId FROM Orders)").run();
  ...
  // Self-heal: Editor role may not exist yet on databases seeded before it
  // was introduced. Idempotent — safe to run on every boot.
  await db.prepare("INSERT INTO Roles (Name) VALUES ('Editor') ON CONFLICT (Name) DO NOTHING").run();
  // Manager: sees the KPI dashboard + operations (orders/invoices/tables) but
  // NOT user management, tax settings, or website content.
  //
  // We want RoleId=4 so lib/auth.js ROLE_MANAGER=4 matches. But forcing an
  // explicit id can collide TWO ways: on Name (already have 'Manager') or on
  // the RoleId PRIMARY KEY (something else already owns id 4). ON CONFLICT
  // targets ONE constraint, so "ON CONFLICT (Name)" does NOT catch a PK
  // clash — that used to throw and 500 the whole site. Guard explicitly:
  // only insert id 4 when neither the name nor id 4 exists; otherwise leave
  // whatever's there. Wrapped in try/catch as a final backstop.
  try {
    const mgr = await db.prepare("SELECT RoleId FROM Roles WHERE Name='Manager'").get();
    if (!mgr) {
      const id4 = await db.prepare("SELECT RoleId FROM Roles WHERE RoleId=4").get();
      if (!id4) {
        await db.prepare("INSERT INTO Roles (RoleId, Name) VALUES (4, 'Manager')").run();
      } else {
        // Id 4 taken by another role — add Manager with an auto id instead.
        await db.prepare("INSERT INTO Roles (Name) VALUES ('Manager') ON CONFLICT (Name) DO NOTHING").run();
      }
    }
    // Keep the SERIAL sequence ahead of any explicit ids we inserted, so a
    // later plain INSERT can't try to reuse an id that already exists.
    await db.prepare("SELECT setval(pg_get_serial_sequence('roles','roleid'), (SELECT MAX(RoleId) FROM Roles))").get();
  } catch (err) {
    console.error("[seed] Manager role setup skipped:", err.message);
  }
  ...
  const content = {
    ...
    hero_title: "Good Coffee.<i>Great Moments.</i>",
    hero_subtitle: "Fresh ingredients, cozy vibes and unforgettable flavors. Welcome to Toast & Roast.",
    hero_image: "",
    site_logo: "",
    popular_title: "Popular Items",
    menu_title: "Our Menu",
    join_title: "Join Your Table",
    join_text: "Enter the host's phone number to find your table and start ordering.",
    join_image: "",
    feature_1_title: "Fresh & Quality",
    feature_1_text: "We use the freshest ingredients daily.",
    feature_2_title: "Fast Service",
    feature_2_text: "Quick preparation and friendly service.",
    feature_3_title: "Cozy Atmosphere",
    feature_3_text: "A perfect place to relax, work or meet.",
    feature_4_title: "Secure & Private",
    feature_4_text: "Your data is safe with us.",
    about_title: "About Toast & Roast",
    ...
    story_1_title: "The Grill", story_1_tag: "Premium cuts, grilled to perfection.", story_1_image: "",
    story_2_title: "The Roastery", story_2_tag: "Expertly roasted beans, brewed with care.", story_2_image: "",
    story_3_title: "The Sweet Finish", story_3_tag: "End your meal with something sweet.", story_3_image: "",
    about_feat_1: "Quality Ingredients", about_feat_2: "Grilled To Perfection",
    about_feat_3: "Specialty Coffee", about_feat_4: "Made With Passion",
    contact_title: "Contact Us",
    contact_lead: "We'd love to hear from you. Reach out to us for reservations, feedback, or any inquiries.",
    contact_address: "Cairo, Egypt",
    map_url: "",
    map_embed: "",
    visit_title: "Visit Us",
    visit_text: "Enjoy our cozy atmosphere, great coffee, and delicious food. We can't wait to welcome you!",
    contact_photo: "",
    ...
    menu_pdf: "",
    invoice_logo: "",
    invoice_branch_line: "Toast & Roast — Cairo, Egypt",
    invoice_footer_note: "Thank you for dining with us",
    theme_primary_color: "#C08A56",
  ...
  // One-time: push the v3 (dark/copper) design over any pre-existing values.
  // The loop above only fills MISSING keys, so without this an existing
  // install would keep its old colours and hero copy. Guarded by a marker
  // row so it runs exactly once and never clobbers later admin edits.
  const v3 = await db.prepare("SELECT 1 FROM WebsiteContent WHERE ContentKey='design_v3_migrated'").get();
  if (!v3) {
    const overrides = {
      theme_primary_color: "#C08A56",
      theme_font_heading: "Playfair Display",
    ...
    for (const [k, v] of Object.entries(overrides)) {
      await db.prepare(`INSERT INTO WebsiteContent (ContentKey,ContentValue) VALUES ($1,$2)
        ON CONFLICT (ContentKey) DO UPDATE SET ContentValue=EXCLUDED.ContentValue`).run(k, v);
    }
    await db.prepare("INSERT INTO WebsiteContent (ContentKey,ContentValue) VALUES ('design_v3_migrated','1')").run();
  }

  // Categories used to share one site-wide note. Seed each existing category
  // with that text once, so nothing disappears on deploy — after this, each
  // category's note is edited (or cleared) independently in Admin.
  const noteBackfill = await db.prepare("SELECT 1 FROM WebsiteContent WHERE ContentKey='category_notes_backfilled'").get();
  if (!noteBackfill) {
    await db.prepare("UPDATE Categories SET Note=$1 WHERE Note IS NULL")
      .run("All prices are subject to a 12% service charge and 14% VAT.");
    await db.prepare("INSERT INTO WebsiteContent (ContentKey,ContentValue) VALUES ('category_notes_backfilled','1') ON CONFLICT (ContentKey) DO NOTHING").run();
  }

  // Focus was previously top/center/bottom only. Translate whatever each
  // category already had into the new Y coordinate so nothing shifts on
  // deploy, then let the sliders take over. Marker-guarded: runs once.
  const focusMig = await db.prepare("SELECT 1 FROM WebsiteContent WHERE ContentKey='image_focus_migrated'").get();
  if (!focusMig) {
    await db.prepare(`UPDATE Categories SET ImageFocusY = CASE ImagePosition
      WHEN 'top' THEN 0 WHEN 'bottom' THEN 100 ELSE 50 END`).run();
    await db.prepare("INSERT INTO WebsiteContent (ContentKey,ContentValue) VALUES ('image_focus_migrated','1') ON CONFLICT (ContentKey) DO NOTHING").run();
  }
  ...
  // One-time backfill: if nothing is marked Featured yet (fresh deploy of
  // this column), seed the same picks the homepage used to hardcode — after
  // this, it's entirely admin-controlled via the Menu Items checkbox.
  const featuredCount = await db.prepare("SELECT COUNT(*) c FROM MenuItems WHERE IsFeatured=true").get();
  if (Number(featuredCount.c) === 0) {
    await db.prepare(`UPDATE MenuItems SET IsFeatured=true WHERE Name IN
      ('Creamy Chicken Casserole','Grilled Fillet 250gm','Nutella Lotus','Seafood Platter','Toast & Roast Burger','V60')`).run();
  }
