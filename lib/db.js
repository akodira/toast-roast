...
async function seed(p) {
  ...
  const content = {
    ...
    hero_title: "The ritual of <i>slow</i> dining",
    hero_subtitle: "From slow-roasted fillets to speciality V60 pours — Cairo's cosiest table is set for you. Come for the coffee, stay for the long conversation.",
    hero_image_1: "",
    hero_image_2: "",
    house_favourites_title: "House Favourites",
    favourites_image: "",
    cuisine_title: "Explore the <i>Cuisine</i>",
    cuisine_image: "",
  ...
  // One-time: apply the new editorial design's colors/fonts/hero copy even
  // over pre-existing values (the loop above only fills in missing keys).
  // Runs exactly once, guarded by a marker row, so it never clobbers
  // whatever the admin customizes going forward.
  const migrated = await db.prepare("SELECT 1 FROM WebsiteContent WHERE ContentKey='design_v2_migrated'").get();
  if (!migrated) {
    const overrides = {
      theme_primary_color: "#C0502A",
      theme_font_heading: "Playfair Display",
    ...
    await db.prepare("INSERT INTO WebsiteContent (ContentKey,ContentValue) VALUES ('design_v2_migrated','1')").run();
  }
