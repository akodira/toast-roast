...
async function seed(p) {
  ...
  // NOT user management, tax settings, or website content. Added later, so
  // self-heal the same way. Explicit ID 4 so lib/auth.js ROLE_MANAGER is
  // always correct even if the serial sequence drifted.
  await db.prepare("INSERT INTO Roles (RoleId, Name) VALUES (4, 'Manager') ON CONFLICT (Name) DO NOTHING").run();
