...
function darken(hex, amt) {
  const n = parseInt((hex || "#A9502F").replace("#", ""), 16);
...
export default async function RootLayout({ children }) {
  ...
  const primary = content.theme_primary_color || "#A9502F";
  const fontHeading = content.theme_font_heading || "Prata";
  const fontBody = content.theme_font_body || "Poppins";
